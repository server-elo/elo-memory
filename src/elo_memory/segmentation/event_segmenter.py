...
class HiddenMarkovEventDetector:
    ...
    def _fallback_clustering(self, observations: np.ndarray) -> List[int]:
        ...
        clustering = AgglomerativeClustering(n_clusters=min(self.n_states, len(observations) // 10))
...
class PredictionErrorDetector:
    ...
    def compute_prediction_error(
        self, observations: np.ndarray, predictions: np.ndarray
    ) -> np.ndarray:
        """
    ...
    def detect_boundaries(self, prediction_errors: np.ndarray) -> List[int]:
        ...
        for i in range(1, len(z_scores) - 1):
            ...
            if (
                z_scores[i] > self.threshold
                and z_scores[i] > z_scores[i - 1]
                and z_scores[i] > z_scores[i + 1]
            ):
                boundaries.append(i)
...
class GraphBoundaryRefiner:
    ...
    def build_similarity_graph(self, observations: np.ndarray, k_neighbors: int = 5) -> nx.Graph:
        """
    ...
    def compute_modularity(self, graph: nx.Graph, boundaries: List[int]) -> float:
        """
    ...
    def compute_conductance(self, graph: nx.Graph, boundaries: List[int]) -> float:
        ...
        for community_id in set(communities):
            ...
            if len(nodes) > 0:
                ...
                external_edges = sum(
                    1 for n in nodes for neighbor in graph.neighbors(n) if neighbor not in nodes
                )
    ...
    def _boundaries_to_communities(self, n_nodes: int, boundaries: List[int]) -> List[int]:
...
class EventSegmenter:
    ...
    def __init__(self, config: Optional[SegmentationConfig] = None):
        ...
        self.pe_detector = PredictionErrorDetector(threshold=self.config.prediction_error_threshold)
        self.graph_refiner = GraphBoundaryRefiner(metric=self.config.modularity_metric)
    ...
    def segment(
        ...
        filtered_boundaries = self._filter_boundaries(initial_boundaries, n_timesteps)
        ...
        if self.config.boundary_refinement and len(filtered_boundaries) > 0:
            refined_boundaries = self.graph_refiner.refine_boundaries(
                observations, filtered_boundaries
            )
        else:
        ...
        confidence_scores = self._compute_confidence(
            observations, refined_boundaries, surprise_values
        )
    ...
    def _filter_boundaries(self, boundaries: List[int], n_timesteps: int) -> List[int]:
        ...
        for i in range(len(boundaries_extended) - 1):
            ...
            if (
                event_length >= self.config.min_event_length
                and event_length <= self.config.max_event_length
            ):
    ...
    def _extract_events(self, observations: np.ndarray, boundaries: List[int]) -> List[np.ndarray]:
    ...
    def _compute_confidence(
        self, observations: np.ndarray, boundaries: List[int], surprise_values: Optional[np.ndarray]
    ) -> List[float]:
